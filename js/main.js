...
;(function(window) {
	...
	DOM.logo = {
		el: document.querySelector('.logo > .logo__inner')
	};
	...
	function init() {
		imagesLoaded(DOM.body, { background: true }, function() {
			...
			pm.loopFx();
			// Glitch effect on some elements (logo, contact and coder link) in the page.
			gfx = new GlitchFx(DOM.glitchElems);
			// Split the logo, contact and code menu items into spans/letters.
			wordsToLetters();
			// Init/Bind events
			initEvents();
		});
	...
	function wordsToLetters() {
		// logo.
		charming(DOM.logo.el);
		DOM.logo.letters = [].slice.call(DOM.logo.el.querySelectorAll('span'));
		// Contact.
	...
	function switchToCode() {
		const hideDesign = function(target, callback) {
			  ...
			  showCode = function(target) {
					...
					if( target === 'logo' || target === 'contact' || target === 'menuCtrl' ) {
						el.classList.remove('mode--design');
						el.classList.add('mode--code');
					}
		...
		// Animate the logo, contact, menu ctrl and menu items out and show the code mode version of these elements.
		// logo:
		hideDesign('logo', function() {
			showCode('logo');
		});
		// Contact:
	...
	function switchToDesign() {
		...
		// Animate the logo, contact, menu ctrl and menu items out and show the design mode version of these elements.
		// logo:
		animateLetters(DOM.logo.letters, 'out', {
			complete: function() {
				showDesign('logo');
			}
		});
